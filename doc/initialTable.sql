...
create procedure authProc (in appid varchar(64), in idcard varchar(18), in curmill bigint, out authflag int)
start_label:begin
	declare houseid char(32);

	select count(1) into authflag from authdb.TAB_AUTH_INFO t2 where t2.HOUSEID = (select t1.houseid from TAB_DEV_INFO t1 where t1.APPID = appid and t1.RECORDSTATUS = 'Y') and t2.IDCARD = idcard;
	if authflag > 0 then
	    select authflag ;
		leave start_label;
	end if;
