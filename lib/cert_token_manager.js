...
function startAutoChangeMode (certTokenSubject) {
    ...
    db.query(sql, [appId], function (schemaInfoRs) {
        ...
        for (var i=0; i<schemaInfoRs.length; ++i) {
            ...
            var startSchemaJob = scheduler.scheduleJob(startTimeCron, changeMode(certTokenSubject, schemaInfo['MODE']));
            var endTimeCromJob = scheduler.scheduleJob(startTimeCron, changeMode(certTokenSubject, 0x40));
            var schemaJob = {};
...
function changeMode(certTokenSubject, mode) {
