...
function CertToken(appId, mode, ws) {
    ...
    this.mode = mode || 0x42;       // 房屋的认证模式（房屋与设备的关系为一对一）
    this.ws = ws;
    this.token = certTokenFactory.createNewToken(5 * appConstants._1min);
    // this.token = certTokenFactory.createNewToken(1 * appConstants._1min);
};
...
function addNewClient(appId, ws) {
    var certTokenSubject = new CertToken(appId, 0x42, ws);
    certTokenSubjectMap[appId] = certTokenSubject;
    process.nextTick(function () {
        getCertToken(appId);
    });
    certIntervalMap[appId] = setInterval(certIntervalExecFun(appId), 5 * appConstants._1min);
    // certIntervalMap[appId] = setInterval(certIntervalExecFun(appId), 1 * appConstants._1min);
    // 为新连接的客户端启动自动切换认证策略定时器
    // TODO startAutoChangeMode(certTokenSubject);
}
...
function removeClient(appId) {
    ...
    delete certTokenSubjectMap[appId];
}
    ...
    var req = https.request(getOptions, function (res) {
        ...
        res.on('end', function () {
            ...
            if (response.ret_code === 0) {
                ...
                // certTimerMap[response['cert_token']] = setTimeout(certTimerExecFun(response.cert_token), 1 * appConstants._1min);
                var ws = curCertTokenSubject.getWs();
                // var clientResp = 'NSIdentifyApp://cert_token=' + response.cert_token + '&mode=' + curCertTokenSubject.getMode();
                var clientResp = '{\"opcode\":\"SNDTOKEN\",\"token\":\"' + response.cert_token + '\",\"mode\":\"' + curCertTokenSubject.getMode() + '\"}';
                console.log("发送给客户端的数据为："  + clientResp);
                ws.send(clientResp)
            } else {
...
exports.setAuthed = function (certToken) {
    var certTokenSubject = certTokenMap[certToken];
    if (certTokenSubject === undefined) {
        return ;
    }
    certTokenSubject.getToken().setAuthed(true);
    getCertToken(certTokenSubject.getAppId());
};
